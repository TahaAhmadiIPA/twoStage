...
float WSCalc() {
	...
	// we store the objective function's value in WSValue variable
	float WSValue = 0;
	...
	for (int s = 0; s < sc; s++) {
		...
		for (int itr = 0; itr < j; itr++) {
			WSobj += establishCost[itr] * xj[itr];
			WSobj += shippingCost[0][itr][0] * z1j1[itr];
			WSobj += shippingCost[0][itr][1] * z1j2[itr];
			WSobj += shippingCost[0][itr][2] * z1j3[itr];
			WSobj += shippingCost[1][itr][0] * z2j1[itr];
			WSobj += shippingCost[1][itr][1] * z1j2[itr];
			WSobj += shippingCost[1][itr][2] * z1j3[itr];
		}
		WS.add(IloMinimize(myenv, WSobj));
		// clearing data
		WSobj.end();
		...
		WS.add(IloRange(myenv, -IloInfinity, con11, factoryCap[0]));
		con11.end();
		...
		WS.add(IloRange(myenv, -IloInfinity, con12, factoryCap[0]));
		con12.end();
		...
		WS.add(IloRange(myenv, 1, con21, 1));
		con21.end();
		...
		WS.add(IloRange(myenv, 1, con22, 1));
		con22.end();
		...
		WS.add(IloRange(myenv, 1, con23, 1));
		con23.end();
		...
		for (int itr = 0; itr < j; itr++) {
			...
			WS.add(IloRange(myenv, -IloInfinity, con31, 0));
			con31.end();
		}
		...
		// updating WSValue
		WSValue += scProb[s] * mycplex.getObjValue();
